...
public class CreateOVXNetwork extends ApiHandler<Map<String, Object>> {
    ...
    public JSONRPC2Response process(final Map<String, Object> params) {
        ...
        try {
            ...
            // SJM NIaaS: Get parameters of srTCM
            final Number typeOfService = HandlerUtils.<Number>fetchField(
            		TenantHandler.TOS, params, true, null);
            final Number ctdInfoRate = HandlerUtils.<Number>fetchField(
            		TenantHandler.CIR, params, true, null);
            final Number ctdBurstSize = HandlerUtils.<Number>fetchField(
            		TenantHandler.CBS, params, true, null);
            final Number exdBurstSize = HandlerUtils.<Number>fetchField(
            		TenantHandler.EBS, params, true, null);
            // SJM NIaaS END
            ...
            final OVXNetwork virtualNetwork = new OVXNetwork(ctrlUrls, addr,
                    netMask.shortValue(),
                    // SJM NIaaS: Set srTCM parameters for OVXNetwork
                    TypeOfService.valueOf(typeOfService.byteValue()),
                    ctdInfoRate.intValue(),
                    ctdBurstSize.longValue(),
                    exdBurstSize.longValue()
                    // SJM NIaaS END
            		);
